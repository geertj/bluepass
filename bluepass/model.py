...
class Model(object):
    ...
    def check_certificate(self, item):
        ...
        try:
            ...
            return False, str(e)
        assert check_uuid4(u[0])
        if not check_uuid4(u[1]):
    ...
    def check_version(self, item):
        ...
        if u[3] and not check_uuid4(u[3]):
            return False, 'Invalid parent UUID "%s"' % u[3]
        if not check_uuid4(u[6]):
